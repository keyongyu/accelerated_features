...
from modules.kymodel import *

# ncnn::Mat out;
# ex.extract("out0", out); /
# std::vector<cv::KeyPoint> keypoints;
# for (int y = 0; y < out.h; y++) {
#     const float* ptr = out.row(y);
#     for (int x = 0; x < out.w; x++) {
#         if (ptr[x] > 0.05f) { // If it passed NMS and Threshold
#             // Create keypoint: x, y, size, angle, response (score)
#             keypoints.push_back(cv::KeyPoint((float)x, (float)y, 5.0f, -1, ptr[x]));
#         }
#     }
# }

# class XFeatNMS(nn.Module):
#     def __init__(self, threshold=0.05, kernel_size=5):
#         super().__init__()
#         self.threshold = threshold
#         self.kernel_size = kernel_size
#         self.pad = kernel_size // 2
#         # MaxPool is a native NCNN layer, very fast!
#         self.max_pool = nn.MaxPool2d(kernel_size=kernel_size, stride=1, padding=self.pad)

#     def forward(self, x):
#         # 1. Local Maxima Map
#         local_max = self.max_pool(x)

#         # 2. Create a Binary Mask (1.0 where it's a keypoint, 0.0 otherwise)
#         # We use float math instead of boolean '&' for better PNNX tracing
#         is_max = (x == local_max).float()
#         is_over_threshold = (x > self.threshold).float()

#         # This mask is (B, 1, H, W) - perfectly static shape!
#         final_mask = is_max * is_over_threshold

#         # Multiply by original values to get (B, 1, H, W) scores at keypoints
#         return x * final_mask
...
class XFeat(nn.Module):
    ...
    def detectAndComputeOrig(self, x, top_k=None, detection_threshold=None):
        ...
        # mkpts = self.NMS_identical(K1h, threshold=detection_threshold, kernel_size=5)
    ...
    def get_heatmap_mask(self, kpt_heatmap, thr=0.05):
        ...
        # mask = (kpt_heatmap == local_max) * torch.relu(kpt_heatmap - 0.2)
        # mask = (kpt_heatmap == local_max) * (kpt_heatmap > 0.2)
